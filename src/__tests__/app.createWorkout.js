...
test('creating the 1a workout', async () => {
  ...
  fireEvent.change(standardsInput, {
    target: {
      value: `Athletes will have 3 minutes to complete each segment - row, bike, burpees. If the athlete does not complete the prescribed work within the 3 minute time cap, their score will be the amount of work completed at the time cap. Atheletes will not advance to the next segment until the alotted 3 minutes has expired. Each segment - row, bike, burpees, is scored individually. Men in both divisions will do the 40 calories on the Assault Bike and women will do 30 calories.`,
    },
  })
  fireEvent.change(rxInput, {
    target: {
      value: '500m Row',
    },
  })
  fireEvent.change(scaledInput, {
    target: {
      value: '500m Row',
    },
  })
